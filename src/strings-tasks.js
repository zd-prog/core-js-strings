...
function countVowels(str) {
  const matches = str.match(/[aeiouyAEIOUY]/gi);
...
function isPalindrome(str) {
  const cleanStr = str.toLowerCase().replace(/[\p{P}\p{S}\s]/gu, '');
...
function reverseWords(str) {
  ...
  words.forEach((word) => {
    result += reverseString(word).concat(' ');
  });
...
function extractNameFromTemplate(value) {
  ...
  return words[1].concat(' ').concat(words[2].replace('!', ''));
}
...
function encodeToRot13(str) {
  const initialSymbols = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const cipheredSymbols =
    'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  ...
  for (let i = 0; i < str.length; i += 1) {
    const symbol = str[i];
    const index = initialSymbols.indexOf(symbol);

    if (index === -1) {
      result = result.concat(symbol);
    } else {
      result = result.concat(cipheredSymbols[index]);
    }
  }
...
function getCardId(value) {
  ...
  const suitValue = 13 * suits[suit];

  if (rank === 'A') return suitValue;

  if (rank === 'J') return 10 + suitValue;

  if (rank === 'Q') return 11 + suitValue;

  if (rank === 'K') return 12 + suitValue;

  return +rank - 1 + suitValue;
}
